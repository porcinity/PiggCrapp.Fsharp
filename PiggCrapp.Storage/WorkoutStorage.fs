...
let connStr =
    "Host=localhost;Database=PiggCrapp;Username=pigg"
    ...
    |> Sql.executeAsync
        (fun read ->
            { WorkoutId = read.uuid "workout_id" |> WorkoutId
              ...
              Variation =
                  read.text "workout_variation"
                  |> WorkoutVariation.fromString
              Exercises = []
...
let findWorkoutAsync workoutId =
    ...
    |> Sql.connect
    |> Sql.query
        "select workout_id, workout_date, workout_variation, user_id
                 from workouts
...
let insertWorkoutAsync workout =
    ...
    |> Sql.parameters [ "@id", Sql.uuid <| WorkoutId.toGuid workout.WorkoutId
                        ...
                        "@variation",
                        Sql.text
                        <| WorkoutVariation.toString workout.Variation
...
let updateWorkoutAsync workout =
    ...
    |> Sql.query
        "update workouts set
                  ...
                  where workout_id = @id"
    |> Sql.parameters [ "@date", Sql.timestamp workout.Date
                        "@variation",
                        Sql.text
                        <| WorkoutVariation.toString workout.Variation
